...
class ImportService
{
    ...
    public function importFromFile($filePath, $jeniLaporan, $userId)
    {
        try {
            ...
            $sheetName = $jeniLaporan === 'signifikan' ? '01B' : '01A';
            $sheetNameLawas = $jeniLaporan === 'signifikan' ? '01A' : '01B';
            
            if (!$spreadsheet->sheetNameExists($sheetName)) {
                // Check if the other sheet exists to provide better error message
                $otherSheetExists = $spreadsheet->sheetNameExists($sheetNameLawas);
                
                if ($otherSheetExists) {
                    $selectedType = $jeniLaporan === 'signifikan' ? 'Signifikan (01B)' : 'Semester (01A)';
                    $actualType = $jeniLaporan === 'signifikan' ? 'Semester (01A)' : 'Signifikan (01B)';
                    return [
                        'success' => false,
    ...
    private function processSingleRow($kodeKomponen, $row, $jeniLaporan, $userId, $importBatchId = null)
    {
        try {
            ...
            $kasus = Kasus::create([
                ...
                'tindak_lanjut_ljk' => $jeniLaporan === 'signifikan' ? $this->getColumnValue($row, 'Tindak Lanjut LJK') : null,
                'source' => 'import',
    ...
    private function processRowGroup($kodeKomponen, $rowGroup, $jeniLaporan, $userId, $importBatchId = null)
    {
        try {
            ...
            $kasus = Kasus::create([
                ...
                'pihak_dirugikan_id' => $pihakDirugikanId,
                'aktivitas_terkait_id' => $aktivitasId,
                'tindak_lanjut_ljk' => $jeniLaporan === 'signifikan' ? $this->getColumnValue($firstRowData, 'Tindak Lanjut LJK') : null,
                'source' => 'import',
                'import_batch_id' => $importBatchId,
            ]);
